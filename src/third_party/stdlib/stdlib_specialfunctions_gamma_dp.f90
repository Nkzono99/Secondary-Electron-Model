module stdlib_dpecialfunctions_gamma_dp
    use ieee_arithmetic, only: ieee_value, ieee_quiet_nan
    use stdlib_kinds, only :  dp, dp, xdp, qp, int8, int16, int32, int64
    ...
    !
        ...
        module procedure gpx_iint16dp   !for integer p and real x
        module procedure gpx_iint32dp   !for integer p and real x
        module procedure gpx_iint64dp   !for integer p and real x
    end interface gpx
    ...
    ! Internal use only
    !
          module procedure l_gamma_iint8dp
          module procedure l_gamma_iint16dp
          module procedure l_gamma_iint32dp
          module procedure l_gamma_iint64dp
...
contains
    ...
    impure elemental function l_gamma_iint8dp(z, x) result(res)
    !
    ...
    !
        ...
        integer(int8) :: i
        integer(int8), parameter :: zero = 0_int8, one = 1_int8, two = 2_int8
        real(dp), parameter :: zero_k2 = 0.0_dp
    ...
    end function l_gamma_iint8dp


    impure elemental function l_gamma_iint16dp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int16), intent(in) :: z
    ...
    end function l_gamma_iint16dp


    impure elemental function l_gamma_iint32dp(z, x) result(res)
    !
    ! Logarithm of gamma function for integer input with defined precision output
    !
        integer(int32), intent(in) :: z
    ...
    ! "Computation of dpecial functions", Shanjie Zhang & Jianmin Jin, 1996, p.48
    ...
    !
        ...
        real(dp) :: res
        integer(int8), parameter :: zero = 0_int8, one = 1_int8, two = 2_int8
        real(dp), parameter :: zero_dp = 0.0_dp, one_dp = 1.0_dp
    ...
    !
        ...
        elseif(x >= -9.0_dp .and. x <= zero_k1) then
        ...
        if(x < zero_k1 .and. p < p_lim .and. abs(anint(p) - p) > tol_dp)   &
    ...
    impure elemental function gpx_iint8dp(p, x) result(res)
    !
    ...
    !
        ...
        else if(x >= -9.0_dp .and. x <= zero) then
        ...
        if(real(p, dp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int8) > (p - two) / two .or. y < b * tol_dp) exit

            end do

            if(y >= b * tol_dp .and. mod(p, two) /= zero_k1)               &
                b = b + d * c / a
    ...
    impure elemental function gpx_iint16dp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, dp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int16) > (p - two) / two .or. y < b * tol_dp) exit
    ...
    impure elemental function gpx_iint32dp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, dp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int32) > (p - two) / two .or. y < b * tol_dp) exit
    ...
    impure elemental function gpx_iint64dp(p, x) result(res)
    !
    ! Approximation of incomplete gamma G function with integer argument p.
    !
    ! Based on Rémy Abergel and Lionel Moisan "Algorithm 1006, Fast and
    ...
    !
        ...
        if(real(p, dp) >= p_lim) then
            ...
            do
                ...
                if(int(n, int64) > (p - two) / two .or. y < b * tol_dp) exit
    ...
    impure elemental function ingamma_low_iint8dp(p, x)          &
        result(res)
    !
    ...
    !
        ...
        else if(x > real(p, dp)) then

            s1 = l_gamma(p, one)
            y = one - exp(-x + p * log(x) - s1) * gpx(p, x)
            res = exp(s1 + log(y))

        else if(x <= real(p, dp) .and. x > zero) then
    ...
    impure elemental function ingamma_low_iint16dp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    impure elemental function ingamma_low_iint32dp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    impure elemental function ingamma_low_iint64dp(p, x)          &
        result(res)
    !
    ! Approximation of lower incomplete gamma function with integer p.
    !
    ...
    impure elemental function l_ingamma_low_iint8dp(p, x)        &
        result(res)
    ...
    impure elemental function l_ingamma_low_iint16dp(p, x)        &
        result(res)
    ...
    impure elemental function l_ingamma_low_iint32dp(p, x)        &
        result(res)
    ...
    impure elemental function l_ingamma_low_iint64dp(p, x)        &
        result(res)
    ...
    impure elemental function ingamma_up_iint8dp(p, x)           &
        result(res)
    !
    ...
    impure elemental function ingamma_up_iint16dp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    impure elemental function ingamma_up_iint32dp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    impure elemental function ingamma_up_iint64dp(p, x)           &
        result(res)
    !
    ! Approximation of upper incomplete gamma function with integer p.
    !
    ...
    impure elemental function l_ingamma_up_iint8dp(p, x)         &
        result(res)
    ...
    impure elemental function l_ingamma_up_iint16dp(p, x)         &
        result(res)
    ...
    impure elemental function l_ingamma_up_iint32dp(p, x)         &
        result(res)
    ...
    impure elemental function l_ingamma_up_iint64dp(p, x)         &
        result(res)
    ...
    impure elemental function regamma_p_iint8dp(p, x) result(res)
    !
    ...
    impure elemental function regamma_p_iint16dp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int16), intent(in) :: p
    ...
    impure elemental function regamma_p_iint32dp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int32), intent(in) :: p
    ...
    impure elemental function regamma_p_iint64dp(p, x) result(res)
    !
    ! Approximation of regularized incomplete gamma function P(p,x) for integer p
    !
        integer(int64), intent(in) :: p
    ...
    impure elemental function regamma_q_iint8dp(p, x) result(res)
    !
    ...
    impure elemental function regamma_q_iint16dp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int16), intent(in) :: p
    ...
    impure elemental function regamma_q_iint32dp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int32), intent(in) :: p
    ...
    impure elemental function regamma_q_iint64dp(p, x) result(res)
    !
    ! Approximation of regularized incomplet gamma function Q(p,x) for integer p
    !
        integer(int64), intent(in) :: p
...
end module stdlib_dpecialfunctions_gamma_dp
